...
режиме payload обрезается, превращаясь в невалидный JSON, который консюмер
отвергнет терминально: данные потеряны с обеих сторон.
...
радости мало. Строка, захваченная воркером старее колонки `claimed_at`, не
имеет отметки времени и считается зависшей: оставить её мог только тот процесс,
которого уже нет.
